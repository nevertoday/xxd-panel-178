...
# XXD Panel 178｜جزيرة مصغرة بطباعة خطية (نفس الأصل)
...
جزيرة مصغرة بطباعة خطية (نفس الأصل)
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 178 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-178`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-178 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-178-prompt.en.md) أو [بالصينية](references/xxd-panel-178-prompt.zh-CN.md).
